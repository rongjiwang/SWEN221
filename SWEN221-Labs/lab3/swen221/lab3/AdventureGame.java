...
public class AdventureGame {
	...
	private Object createObject(GameFile.Item item) {
		...
		case "SecretButton":
			return createSecretButton(item);
		default:
	...
	/**
	 * Create locked door within the game.
	 * 
	 * @param item
	 * @return
	 */
	private LockedDoor createLockedDoorObject(GameFile.Item item){
		...
		System.out.println(code+" "+from+" "+to);
		Room oneSide = (Room) objects[from];
		Room otherSide = (Room) objects[to];
	...
	private SecretButton createSecretButton(GameFile.Item item){
		int from = (Integer) item.field("location");
		int code = (Integer)item.field("code");
		Room oneSide = (Room) objects[from];
		SecretButton s = new SecretButton(oneSide, code);
		oneSide.addItem(s);
		return s;
	}
