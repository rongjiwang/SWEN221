...
/**
 * Responsible for validating that a given game adheres to the rules of
 * Minesweeper.
 * 
...
public class Game {
	...
	/**
	 * Place a bomb on the board. This should be done before calling the
	 * initialise method.
	 * 
	 * @param position
	...
	public int getF() {
		...
		for (int y = 0; y != height; ++y) {
			for (int x = 0; x != width; ++x) {
				Square sq = board[y][x];
				if (sq.isHidden() && sq instanceof BlankSquare && sq.isFlagged()) {
					numberOfHiddenBlanks++;
				}
			}
		}
		return numberOfHiddenBlanks;
	}
	public int getB() {
		int numberOfHiddenBlanks = 0;
