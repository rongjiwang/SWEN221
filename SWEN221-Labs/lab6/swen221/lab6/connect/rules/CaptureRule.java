...
public class CaptureRule implements Rule {
	...
	private boolean isCaptured(Board original, Board.Token token, int x, int y) {
		if(isHorizontalLongCapture(original, token, x, y) || isHorizontalShortCapture(original, token, x, y)){return true;}
		else if(isVerticalLongCapture(original, token, x, y) || isVerticalShortCapture(original, token, x, y)){return true;}
	...
	private boolean isHorizontalLongCapture(Board original, Board.Token token, int x, int y) {
		if (x == 1 || x == 2) {
			...
			if(original.getSquare(left) == null || original.getSquare(right) == null){return false;}
	...
	private boolean isVerticalLongCapture(Board original, Board.Token token, int x, int y) {
		if (y == 1 || y == 2) {
			...
			Board.Token opp = getOpposite(token);
			if(original.getSquare(top) == null || original.getSquare(bottom) == null){return false;}
