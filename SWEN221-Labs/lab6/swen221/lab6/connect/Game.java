...
public class Game {
	...
	public Status getStatus() {
		ColumnWinRule col = new ColumnWinRule();
		RowWinRule row = new RowWinRule();
		StaleMateRule sta = new StaleMateRule();
		if(col.apply(this)!= Status.ONGOING){return col.apply(this);}
		if(row.apply(this)!= Status.ONGOING){return row.apply(this);}
		if(sta.apply(this)!= Status.ONGOING){return sta.apply(this);}
	...
	public void placeToken(Position pos, Board.Token token) {
		Status s = getStatus();
		if(s==Status.BLACKWON || s==Status.WHITEWON || s==Status.STALEMATE){
			throw new IllegalArgumentException("The End of the game, no more move allowed");
		}
		if(isWhitesTurn() && token == Board.Token.BLACK) {
			throw new IllegalArgumentException("Illegal move: not black player's turn");
		}  if(isBlacksTurn() && token == Board.Token.WHITE) {
			throw new IllegalArgumentException("Illegal move: not white player's turn");
		}  if(board.getSquare(pos) != null) {
		...
		moves = moves + 1;
//		CaptureRule cap = new CaptureRule();
//		cap.apply(this);
		for(Rule rule : rules) {
